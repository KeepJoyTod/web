...
## 2.3 macOS CI、目标机验收与边界

`.github/workflows/deployment-check.yml` 定义托管 macOS Runner Job `macos-dry-run`。其配置 JDK 17 和 `.nvmrc` Node.js，执行 `scripts/*.sh` 与 `back/mvnw` 的 Bash 语法检查，断言 `macos-architecture`、`macos-bash`、`macos-ps-identity` 的 doctor 输出，执行 `init-local-db.sh --dry-run`、`deploy.sh --dry-run --skip-infrastructure`，然后编译后端并构建两个前端。doctor 的退出码不是该 Job 的成功前提，因为托管 Runner 的 Docker 可用性不属于工作流契约。

该 Job 只在 GitHub Actions 实际执行并成功后才是 macOS 验证证据；工作流配置或本地差异均不能说明它已经运行。它也不启动 Docker Compose 或应用进程，不能覆盖 Docker Desktop 图形界面、bind mount 目录共享、真实镜像拉取、容器完整启动，以及第 6 节的三个 HTTP 验证。

每台首次部署的目标 macOS 必须完成一次人工验收：

1. 确认 Docker Desktop 正在运行，且项目根目录及第 2.2 节列出的 bind mount 目录可共享访问。
2. 执行 `bash ./scripts/doctor.sh`。仅在默认 Docker 流程所需检查通过后继续；否则按第 4 节改用已确认的本机 MySQL/Redis 流程。
3. 目标数据库没有表时执行 `bash ./scripts/deploy.sh --init-db`；目标库已有表时执行 `bash ./scripts/deploy.sh`。不明确时停止，不要通过清库、删表或删除 volume 强行重试。
4. 按第 6 节确认后端健康状态为 `UP` 且两个前端均为 HTTP `200`，并保留实际命令与日志位置。

自托管 macOS Runner 是可选的持续验证手段，只应由项目所有者在需要覆盖特定 Mac 硬件、网络或 Docker Desktop 行为时决定并维护。不得自动创建、注册或向其配置真实凭据；除非工作流显式新增 Docker 与完整启动测试，它仍不能取代上述目标机验收。
...
```bash
health="$(curl --silent --show-error --fail http://127.0.0.1:8080/api/actuator/health)"
node -e 'const health = JSON.parse(process.argv[1]); const ready = health.status === "UP" && health.components?.db?.status === "UP" && health.components?.redis?.status === "UP"; if (!ready) process.exit(1); console.log("health=UP");' "$health"
user_status="$(curl --silent --show-error --output /dev/null --write-out '%{http_code}' http://127.0.0.1:5173/)"
admin_status="$(curl --silent --show-error --output /dev/null --write-out '%{http_code}' http://127.0.0.1:5174/)"
[[ "$user_status" == "200" && "$admin_status" == "200" ]] || { printf 'user=%s admin=%s\n' "$user_status" "$admin_status" >&2; exit 1; }
printf 'health=UP user=%s admin=%s\n' "$user_status" "$admin_status"
```

期望：`health=UP user=200 admin=200`。`curl` 或 Node 命令失败时，查看第 7 节对应的应用日志；不要仅根据端口监听判断成功。
